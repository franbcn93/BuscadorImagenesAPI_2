...
export const Formulario = ({ guardarBusqueda, guardarImgPorPagina }) => {
  ...
  const buscarImagenes = (e) => {
    ...
    if (termino.trim() === "" || paginas.trim() === "" || isNaN(cambioEntero)) {
      guardarError(true);
      return;
    }
  ...
  return (
    <form action="" onSubmit={buscarImagenes}>
      ...
      {error ? (
        <Error mensaje="Agrega un término de búsqueda y una cantidad de imagenes para ver en la misma página" />
      ) : null}
    </form>
  );
};
